...
namespace UefaServiceV4.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<UefaServiceV4.Models.UefaServiceV4Context>
    {
        ...
        protected override void Seed(UefaServiceV4.Models.UefaServiceV4Context context)
        {
           ...
           // context.RoundOnes.AddOrUpdate(x => x.Id,
             //   new RoundOne() { Id = 1, TeamName = ""


            Assembly roundOneAssembly = Assembly.GetExecutingAssembly();
            string roundOneResourceName = "UefaServiceV4.App_Data.RoundOne.csv";
            using (Stream stream = roundOneAssembly.GetManifestResourceStream(roundOneResourceName))
            {
                using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
                {
                    CsvReader csvReader = new CsvReader(reader);
